                ...
                ?>
                    <div class="d-flex w-100 justify-content-between  mb-2 py-2 border-bottom cart-item">
                        <div class="d-flex align-items-center col-8">
                            <span class="mr-2"><a href="javascript:void(0)" id="dataX" class="btn btn-sm btn-outline-danger rem_item" data-id="<?php echo $row['id'] ?>"><i class="fa fa-trash"></i></a></span>
                        ...
                        <div class="col text-right align-items-center d-flex justify-content-end">
                            ...
                            $x =$row['pid'];
                            $q = $conn->query("SELECT * FROM products where id = '$x' ");
                            $row1= $q->fetch_assoc();
                            $price = $row['price'] * $row['quantity'] ;
                            $quan = 0;
                            $total =0;
                            $queryC = $conn->query("SELECT * FROM cart");
                            while($rowC = $queryC->fetch_assoc()):
                                $quan += $rowC['quantity'];
                                $prd_inv = $rowC['inventory_id'];
                                $query2 = $conn->query("SELECT * FROM inventory where id = '$prd_inv'");
                                $row2 = $query2->fetch_assoc();
                                $prd_inv2 = $row2['product_id'];
                                $query3 = $conn->query("SELECT * FROM products where id = '$prd_inv2'");
                                $row3 = $query3->fetch_assoc();
                                $prd_cat = $row3['category_id'];
                                $discount = 0;
                                if ($prd_cat == 1){
                                    if($quan > 48){
                            ...
                            <h4><b class="total-amount"><?php echo number_format($price)?></b></h4>
                            
                        </div>
                    </div>
                <?php endwhile; ?>
                <div class="d-flex w-100 justify-content-between mb-2 py-2 border-bottom">
                    <div class="col-8 d-flex justify-content-end"><h4>Grand Total(+7000 for delivery):</h4></div>
                    <div class="col d-flex justify-content-end"><h4 id="grand-total">-</h4></div> 
                    <h4><br>Discount By :<b class="total-discount"><?php echo number_format($discount)?></b>%</h4>
                </div>
            </div>
        </div>
        <div class="d-flex w-100 justify-content-end">
            <a href="./?p=checkout&hddxaspoiwsskali=<?php echo $discount?> " class="btn btn-sm btn-flat btn-dark">Checkout</a>
        </div>
    </div>
</section>
<script>
